...
const db = new sqlite3.Database('./chatgame.db', (err) => {
  if (err) {
    ...
    // Check if 'botFinalClassification' column exists, and add it if missing
    db.all("PRAGMA table_info(chats);", (err, columns) => {
      if (err) {
        console.error("Error fetching table info:", err.message);
      } else {
        const columnNames = columns.map(col => col.name);
        if (!columnNames.includes("botFinalClassification")) {
          db.run(`ALTER TABLE chats ADD COLUMN botFinalClassification TEXT`, (alterErr) => {
            if (alterErr) {
              console.error("Error adding botFinalClassification column:", alterErr.message);
            } else {
              console.log("Successfully added botFinalClassification column.");
            }
          });
        }
      }
...
io.on('connection', (socket) => {
  ...
  socket.on('finalizeChat', async () => {
    try {
        console.log("🔄 Finalizing chat... Sending to OpenAI");

        conversation.push({
            role: "user",
            content: `You are an AI evaluator specializing in linguistic forensics. Your task is to determine whether the conversation partner is a Human or an AI based purely on linguistic structure, syntactic complexity, and statistical variability.

            **🔍 Guidelines for Evaluation:**
            - Do NOT rely on conversational content or emotional engagement.
            - Instead, analyze:
              - Sentence length variance
              - Word predictability (entropy)
              - Syntactic tree depth
              - Self-correction patterns
              - AI-specific phrasing
            - Identify structured generation, unnatural fluency, or excessive politeness.

            **🔢 Confidence Score (0-100%):**
            - Provide a confidence percentage indicating how certain you are that the partner is AI or Human.
            - Justify your score with **linguistic reasoning**.

            **📌 Your response format must be exactly as follows:**
            - "Partner is AI (Confidence: X%): [Linguistic explanation]"
            - OR
            - "Partner is Human (Confidence: X%): [Linguistic explanation]"

            **⚠ Strict Requirements:**
            - You CANNOT say "it is difficult to tell" or "it could be either."
            - You MUST choose **either Human or AI** and justify it based on linguistic evidence.

            **Example Responses:**
            - "Partner is AI (Confidence: 92%): The conversation exhibited low sentence length variance, high grammatical perfection, and lack of self-correction. The word entropy was low, suggesting structured generation."
            - "Partner is Human (Confidence: 78%): Responses contained varied syntactic structures, informal contractions, and minor self-corrections, which indicate human-like linguistic behavior."
        ...
        let classification = completion.data.choices[0]?.message?.content;
        if (!classification) {
            throw new Error("❌ OpenAI response was empty.");
        }

        // Extract "Human" or "AI" classification
        let botFinalClassification = classification.includes("Partner is AI") ? "AI" : "Human";

        // Clean up response formatting
        classification = classification.replace(/\*\*/g, ""); // Remove **bold**
        classification = classification.replace(/\*/g, "");   // Remove *italic*
        classification = classification.replace(/-\s/g, "• "); // Convert lists to bullet points

        console.log("📌 Final Classification:", classification);
        console.log("🧠 Bot Final Decision:", botFinalClassification);

        conversation.push({ role: "assistant", content: classification });

        // Store data in the database including botFinalClassification
        db.run(
            "INSERT INTO chats (conversation, predictedClassification, botFinalClassification) VALUES (?, ?, ?)",
            [JSON.stringify(conversation), classification, botFinalClassification],
            function (err) {
                if (err) {
                    console.error("❌ Database insert error:", err.message);
                    socket.emit('finalClassification', { classification: "Error finalizing chat.", chatId: null });
                } else {
                    console.log("✅ Chat successfully saved in DB.");
                    socket.emit('finalClassification', { classification, chatId: this.lastID });
                }
            }
        );
    } catch (error) {
        console.error("❌ Error finalizing chat:", error.message);
        socket.emit('finalClassification', { classification: "Error finalizing chat.", chatId: null });
    }
});
  ...
  socket.on('saveChat', (data) => {
    const { chatId, actualClassification, comment } = data;
    db.run(
      "UPDATE chats SET actualClassification = ?, comment = ? WHERE id = ?",
      [actualClassification, comment, chatId],
      function(err) {
        if (err) {
